...
class ItineraryManager:
    ...
    def __init__(
        ...
        self.locked_per_day: Dict[date,
                                  List[Activity]] = {}  # Specific day locks
        # User doesn't want these
        self.excluded_activities: Set[Activity] = set()
    ...
    def lock_activity(self, activity, specific_day=None):
        ...
        **Returns**
        ...
        if specific_day:
            ...
            print(
                f"🔒 Locked '{
                    activity.name}' (will appear somewhere in itinerary)")
    ...
    def regenerate_itinerary(self, scheduler_func):
        ...
        **Returns**
        ...
        self.current_itinerary = scheduler_func(
            self.trip, available, self.prefs, list(
        ...
        if missing_locked:
            print(
                f"⚠️  Warning: Could not fit {
                    len(missing_locked)} locked activities:")
    ...
    def swap_activity(self, old_activity, new_activity, day_index):
        ...
        **Returns**
        ...
        if new_activity.duration_hours > time_freed:
            print(
                f"⚠️  '{
                    new_activity.name}' is longer than '{
                    old_activity.name}'")
        ...
        print(
            f"✅ Swapped '{
                old_activity.name}' → '{
                new_activity.name}' on day {day_index}")
        return True
    ...
    **Returns**
    ...
    while True:
        ...
        if choice == '1':
            ...
            activity = next(
                (a for a in manager.all_activities if a.name.lower() == activity_name.lower()),
                None)
            if activity:
            ...
            activity = next(
                (a for a in manager.locked_activities if a.name.lower() == activity_name.lower()),
                None)
            if activity:
            ...
            activity = next(
                (a for a in manager.excluded_activities if a.name.lower() == activity_name.lower()),
                None)
            if activity:
            ...
            # Save and exit
            filename = f"itinerary_{
                manager.trip.destination}_{
                manager.trip.start_date}.json"
